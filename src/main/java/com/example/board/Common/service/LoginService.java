...
// 3. 해당 메서드에서 DB에서 조회한 user객체를 만들어서 return 해줘야함.
// 4. 스프링에서 DB의 user객체와 사용자가 입력한 email/password를 비교
// 5. 검증이 완료되면, DB의 user객체를 Authentication객체에 저장(많이 중요!!!)
@Service
public class LoginService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        Author author = authorRepository.findByEmail(username).orElseThrow(()->new EntityNotFoundException("aaa"));
